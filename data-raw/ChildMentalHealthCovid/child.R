## Code to prepare `child` dataset.

# Source the review-specific eligibility criteria.
source(file.path("data-raw", "ChildMentalHealthCovid", "criteria.R"), local = FALSE)
...
raw <- read.csv(
    file.path("data-raw", "ChildMentalHealthCovid", "data.csv"),
    check.names = FALSE
)
...
testthat::expect_equal(nrow(child), 13702L)
testthat::expect_equal(sum(child$included), 422L)

# Save the dataset and criteria object in the package data directory.
usethis::use_data(child, child_criteria, overwrite = TRUE, compress = "xz")
