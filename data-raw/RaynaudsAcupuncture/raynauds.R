## Code to prepare `raynauds` dataset.

# Source the review-specific eligibility criteria.
source(file.path("data-raw", "RaynaudsAcupuncture", "criteria.R"), local = FALSE)
...
raw <- read.csv(
    file.path("data-raw", "RaynaudsAcupuncture", "data.csv"),
    check.names = FALSE
)
...
testthat::expect_equal(nrow(raynauds), 948L)
testthat::expect_equal(sum(raynauds$included), 6L)
